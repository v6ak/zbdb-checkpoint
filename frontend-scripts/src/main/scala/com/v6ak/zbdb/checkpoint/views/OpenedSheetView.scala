...
import com.v6ak.zbdb.checkpoint.routing.{AboutState, RoutingState, SelectSheetState, StatisticsState, TableState}
import io.udash.{ContainerView, _}
import org.scalajs.dom.Element
import scalatags.JsDom.all._
import scalatags.{JsDom, generic}

class OpenedSheetView(sheetId: String)(implicit applicationContext: ApplicationContext) extends ContainerView {
    ...
    nav(cls := "mdc-drawer__drawer")(
      header(cls := "mdc-drawer__header")(
        div(cls := "mdc-drawer__header-content")(
          Todo.sheetTitle(sheetId)
          //div("Stanoviště č.")(applicationContext.pos, ": ", applicationContext.name)
        )
      ),
      section(cls := "mdc-drawer__content", id := "main-menu")(
        nav(cls := "mdc-list")(
          item(TableState(sheetId), "list", "Tabulka"),
          item(StatisticsState(sheetId), "show_chart", "Statistiky"),
        ),
        hr(cls := "mdc-list-divider"),
        nav(cls := "mdc-list")(
          item(SelectSheetState, "swap_horiz", "Změnit stanoviště"),
        ),
        hr(cls := "mdc-list-divider"),
        nav(cls := "mdc-list")(
          item(AboutState(sheetId), "info", "O aplikaci"),
        )
      )
    )
  ).render
